...
async function _Generate_Claim_Pack(_Claim_Id, _Data) {
    ...
    const _Claim_Form_PDF = await _Build_Claim_Form_PDF(_Claim_Id, _Data);
    const _Evidence_Report_PDF = await _Build_Evidence_Report_PDF(_Claim_Id, _Data);
    const _Cover_Letter_PDF = await _Build_Cover_Letter_PDF(_Claim_Id, _Data);
    const _Bundled_Pack_PDF = await _Bundle_PDFs(_Cover_Letter_PDF, _Claim_Form_PDF, _Evidence_Report_PDF);

    const [_Key_Form, _Key_Evidence, _Key_Cover, _Key_Pack] = await Promise.all([
    ...
    const _Pack_URL = await _S3_Helper._Get_Presigned_URL(_Key_Pack, 7 * 24 * 3600);
